...
{
    public abstract class DisasterReportTestBase : AbpIntegratedTestBase<DisasterReportTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected DisasterReportTestBase()
        {
        ...
        protected void UsingDbContext(Action<DisasterReportDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<DisasterReportDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<DisasterReportDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<DisasterReportDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<DisasterReportDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<DisasterReportDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<DisasterReportDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<DisasterReportDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<DisasterReportDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<DisasterReportDbContext, Task<T>> func)
        {
            T result;
