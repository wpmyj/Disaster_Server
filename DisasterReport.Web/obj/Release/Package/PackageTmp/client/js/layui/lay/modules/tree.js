...
layui.define('jquery', function (exports) {
  ...
  Tree.prototype.tree = function (elem, children) {
    ...
    layui.each(nodes, function (index, item) {
      ...
      var li = $(['<li ' + (item.spread ? 'data-spread="' + item.spread + '"' : '') + '>'
        ...
        , function () {
          return options.check ? (
            ...
            '<i class="layui-icon layui-tree-check">' + (
              options.check === 'checkbox' ? '<input type="checkbox" name="layui-tree-checkbox" />' : (
                options.check === 'radio' ? '<input type="radio" name="layui-tree-radio" />' : ''
              )
            ) + '</i>'
          ) : '';
        } ()
        ...
        , function () {
          return '<a href="' + (item.href || 'javascript:;') + '" ' + (
            ...
            + ('<cite style="'+ item.name[1] +'">' + (item.name[0] || '未命名') + (item.addition ? ('——(' + item.addition + ')') : '') + '</cite></a>');
        } ()
      ...
      //触发checkbox/radio节点回调
      typeof options.chkClick === 'function' && that.chkClick(li, item);
  ...
  Tree.prototype.click = function (elem, item) {
    ...
    elem.children('a').on('click', function (e) {
      layui.stope(e);
      options.click.call(this, item)
    });
  };

  //点击checkbox/radio回调
  Tree.prototype.chkClick = function (elem, item) {
    var that = this, options = that.options;
    elem.parent().children('input').on('click', function (e) {
      layui.stope(e);
      options.click.call(this, item)
    });
  };
